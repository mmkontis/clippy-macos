...
The recent-media stack scrolls down to reveal and up to hide. Hiding preserves both its session queue and clipboard history; a new media copy reveals it again. The horizontal shelf opened with Clipboard hides on a vertical scroll in either direction and still supports horizontal scrolling. Pointer movement into a media preview no longer collapses the expanded stack, and hiding an empty stack cannot race with a newly copied tile. Media tiles keep fixed hover areas while magnifying. One wheel notch or a deliberate trackpad scroll operates the stack; momentum cannot reopen it. Dismissing a tile also hides it from expanded history for this session, without deleting the clip. Command-V dismisses only the media belonging to the current pasteboard, including a copy made just before the monitor polls. Text and repeated pastes do not consume unrelated tiles. Remaining device-specific verification is tracked in issue #2.
...
A Debug build accepts `--self-test-windows` for native window regression checks against isolated synthetic history. It verifies Settings click/focus dismissal, rapid reopening, and empty-media-shelf dismissal without starting clipboard monitoring or registering a login item. The Mac must be unlocked and the test app in the foreground. The test waits for foreground activation before asserting focus changes, because macOS can deny activation to a background CLI launch. The process exits after the checks; restart the normal app afterward.
